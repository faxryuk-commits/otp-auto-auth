...
export async function handleAuthVerify({ body, ip, userAgent }: VerifyParams) {
  ...
  if (channel === 'wa') {
    ...
    const waPhone = phone ?? session.phone ?? undefined;
    ...
    user = await prisma.user.upsert({
      ...
      update: {
        phone: session.phone ?? phone ?? undefined,
        updatedAt: new Date(),
      },
      create: {
  ...
  const token = await createJwt(
    { sub: user.id, channel: channel === 'wa' ? 'wa' : 'tg' },
    { setCookie: true },
  );
...
function ensureProviderEnabled(provider: 'wa' | 'tg-otp') {
